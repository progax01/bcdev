...
// Load the ABI and address of your test DAI token contract
...
const Dai = new web3.eth.Contract(daiTokenAbi, daiTokenAddress);

// Select a Ganache account for testing (replace with your desired account)
const testingAccount = '0xF5f710AFb37b4BedcB6faA820dcEd669cb0fF8eC';

// Mint test DAI tokens to the testing account
const amountInWei = web3.utils.toWei('10000', 'ether'); // Mint 1000 test DAI tokens

//const amountInWei = web3.utils.toWei(transferAmount.toString(), "ether"); // Convert to Wei

const transferParams = {
  from: daiTokenAddress,
  to: testingAccount,

  gas: 500000,
  data: Dai.methods.mint(recipientAddress, amountInWei).encodeABI(),
};

// const transaction = await tokenContract.methods.transfer(transferParams.to, transferParams.value).send({ from: account[0]});

const txhash = web3.eth.sendTransaction(transferParams);
console.log(`Mint transaction hash: ${txhash}`);

// daiTokenContract.methods.mint(testingAccount, amountInWei)
//   .send({ from: testingAccount })
