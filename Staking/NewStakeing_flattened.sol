...
contract MyStakingContract {
    ...
    function stakenow(uint amount, uint duration ) public returns (Stake memory ){
        ...
        Stake memory newStake = Stake(tokenid, amount, duration, block.timestamp, msg.sender,false,false);
        userStakes[msg.sender].push(newStake);
        bool success= token.transferFrom(msg.sender, address(this), amount);
        require(success==true, "Transfer Failed");
        cid[tokenid]=newStake;
        tokenIdCounter.increment();
        return newStake;

    } 
    
    function checkDuration() public view returns (uint256[] memory) {
        return time;
    }
 ...
 function calculateReward(uint256 userid) public view returns (address,uint256, uint256, uint256)
    ...
    function claim_reward(uint256 userid) public {
